...
2. We have discussed before that the conventional $\alpha$ threshold we use to determine significance given the null represents the probability of a type I or false positive error we tolerate when conducting a significance test. We will examine how that probability is affected by conducting multiple tests between fully independent samples drawn from the same null distribution.

  * Study the `fwer_independent` tibble. Use ggplot to show how the probability of a type I error `fwer` changes as the number of `total_tests` increases. After approximately how many tests is the probability of a type I error > 0.5? 
...
```
  ...
  * Now `mutate` a new column in `fwer_independent` that divides `mean_n_type_I`, the number of type I errors committed, by `total_tests`. The proportions should stay at around 5% as we increase the number of total tests we run. That means we can expect the number of type I errors to increase linearly at 5% of the total number of comparisons we run. Why is this a problem for drawing conclusions from our data?
...
```
* Run the following code and inspect the `fwer` and `mean_power` trends under each method. We will visualize this a bit more clearly in the following code block.
```{r}
#| eval: FALSE
